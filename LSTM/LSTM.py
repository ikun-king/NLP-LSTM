...
class TextLSTM(nn.Module):
    ...
    def forward(self, X):
        ...
        c_t = c_0
        for x in X:
          i_t = self.sigmoid(self.W_ii(x) + self.b_ii + self.W_hi(h_t) + self.b_hi)
          f_t = self.sigmoid(self.W_if(x) + self.b_if + self.W_hf(h_t) + self.b_hf)
          g_t = self.tanh(self.W_ig(x) + self.b_ig + self.W_hg(h_t) + self.b_hg)
          o_t = self.sigmoid(self.W_io(x) + self.b_io + self.W_ho(h_t) + self.b_ho)
          c_t = torch.mul(f_t,c_t)+torch.mul(i_t,g_t)
          h_t = torch.mul((o_t),self.tanh(c_t))
        model_output = nn.functional.softmax(self.W(h_t) + self.b)
        '''end'''
